...
void judge_fork() {
    if (fork_flag.find_fork == 0) {
        ...
        if (!block_judge(0 + ROW / 5, 5 + ROW / 5, COL / 2 - 2, COL / 2 + 3, 20,
                         0))
            return;
        if(connect.room!=6||connect.room_cnt<=2)return;
        int useful = 0;
        for (int i = 0; i < 10; i++) {
        ...
        if (Img.RightBorder[ROW / 2] - Img.LeftBorder[ROW / 2]
            < RoadWide0 * 2)
            return;
        int16 k_error = 4;
        ...
        if(connect.room!=4||connect.room_cnt<=2 )return;
        int useful = 0;
        ...
        if (useful < ROW*0.7 - 5)return;
        fork_flag.on_fork = 1;
    } else if (fork_flag.out_fork == 0) {
        if(connect.room!=6||connect.room_cnt<=1)return;
//        if (!block_judge(ROW - 1 - 5, ROW - 1, 0, 5, 20, 0))
//            return;
//        if (!block_judge(ROW - 1 - 5, ROW - 1, COL - 1 - 5, COL - 1, 20, 0))
//            return;
...
void judge_circle_right() {
    if (circle_flag.find_right_circle == 0) {
        temp_flag = 0;
        if(connect.room<4||connect.room>=6)return;
...
//                return;
            ...
            {
                if(Img.LeftBorder[i]<Img.LeftBorder[i+5]) block_cnt++;
            }

            if(block_cnt<max_py-20)return;
...
void judge_circle_left() {
    if (circle_flag.find_left_circle == 0) {
        temp_flag = 0;
        if(connect.room<4||connect.room>=6||connect.room_cnt<=1)return;
