...
uint8 GetBorder(    uint8_t img_tmp[ROW][COL]) {
    ...
    for (Ysite = StartScanRow; Ysite < ROW; Ysite++) {
        ...
        if (Img.RoadWide[Ysite] < RoadWide0 - 7
            && Base.ToplineFindFlag == 'F') {
            Base.ToplineTemp = Ysite;
            Base.ToplineFindFlag = 'T';
        }
        if (Ysite == ROW - 1 && Base.ToplineFindFlag == 'F') {
            Base.ToplineTemp = (max_py + max_py1) / 2;
        }
        Base.LoseRoadLine = max_py;
    }
    Base.ToplineTemp = max(max_py, max_py1);

    Base.Topline = Base.ToplineTemp;    //最终顶线更新
    int last_find=0,prev_find=0;
    if(circle_flag.find_left_circle||circle_flag.find_right_circle)
    {
        int min_find=COL;
        for(int i=0;i<ROW;i++)
        {
            int find=0,black_cnt=0,start_X;
            for(int j=(Img.LeftBorder[i]<lose_error?lose_error:Img.LeftBorder[i]);j<(Img.RightBorder[i]>COL-1-lose_error?COL-1-lose_error:Img.RightBorder[i]);j++)
            {
                if(ImageUsed[ROW-1-i][j]==ImageUsed[ROW-1-i][j+1]&&
                   ImageUsed[ROW-1-i][j+2]==ImageUsed[ROW-1-i][j+3]&&
                   ImageUsed[ROW-1-i][j]!=0&&  ImageUsed[ROW-1-i][j+2]!=  ImageUsed[ROW-1-i][j]
                   &&find==0&&(
                           img_tmp[i][j]||img_tmp[i][j+1]||img_tmp[i][j+2]||img_tmp[i][j+3]

                   )) { start_X=j+2,find=1;}
                else if(find==1&&ImageUsed[ROW-1-i][j]==0)  black_cnt++;
                else if(find==1&&ImageUsed[ROW-1-i][j]==ImageUsed[ROW-1-i][j+1]&&ImageUsed[ROW-1-i][j]!=0)
                {
                    find=2;
                    if(i<min_find)
                    {
                        min_find=i;
                        Base.first_left_find_to_lose_y=i;
                        Base.first_left_find_to_lose_x=start_X+(black_cnt-1)/2;
                        Base.first_right_find_to_lose_y=i;
                        Base.first_right_find_to_lose_x=start_X+(black_cnt-1)/2;
                    }
                    break;
                }
            }
            if(find!=2&&last_find==2&&prev_find==2)break;
            prev_find=last_find;
            last_find=find;
        }
        if (Base.first_left_find_to_lose_y == 0 && Base.first_right_find_to_lose_y == 0
            && (circle_flag.find_left_circle || circle_flag.find_right_circle)) {
            Base.first_left_find_to_lose_y = ROW - 1;
            Base.first_right_find_to_lose_y = ROW - 1;
            Base.first_left_find_to_lose_x = Img.RightBorder[ROW - 1]
                                             -RoadWide0/2;
            Base.first_right_find_to_lose_x = Img.LeftBorder[ROW - 1]
                                              +RoadWide0/2;
        }
    }
    return 1;
